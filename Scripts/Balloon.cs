...
public partial class Balloon : CanvasLayer {
    ...
    public override void _Ready() {
        ...
        balloon.GuiInput += (@event) => {
            if ((bool)dialogueLabel.Get("is_typing")) {
                bool mouseWasClicked =
                    ...
                    (@event as InputEventMouseButton).ButtonIndex ==
                        MouseButton.Left &&
                    @event.IsPressed();
        ...
        responsesMenu.Connect("response_selected",
                              Callable.From((DialogueResponse response) => {
    ...
    public override async void _Notification(int what) {
        // Detect a change of locale and update the current dialogue line to
        // show the new language
        if (what == NotificationTranslationChanged &&
            IsInstanceValid(dialogueLabel)) {
            float visibleRatio = dialogueLabel.VisibleRatio;
            DialogueLine = await DialogueManager.GetNextDialogueLine(
                resource, DialogueLine.Id, temporaryGameStates);
    ...
    public async void Start(Resource dialogueResource, string title,
        ...
        DialogueLine = await DialogueManager.GetNextDialogueLine(
            resource, title, temporaryGameStates);
    }

    public async void Next(string nextId) {
        DialogueLine = await DialogueManager.GetNextDialogueLine(
            resource, nextId, temporaryGameStates);
    }
...
#region Helpers
    ...
    private async void ApplyDialogueLine() {
        ...
        if (dialogueLine.Character == "Nike") {
            GameState.GetGSInstance().talking = true;
        } else {
            GameState.GetGSInstance().talking = false;
        }
